...
alone are missing recent DLA rows that the archived pull supplies. (Note:
backfilling live rather than replaying the archive could change downstream
numbers if any FPDS records have been revised since the thesis snapshot.)
    ...
    return [
        ...
        Stage("backfill_replay", "merge archived API rows into the parquet (OFFLINE)",
              ...
              note=("OFFLINE REPLAY of the archived API pull — authoritative for "
                    "reproduction. (Backfilling live rather than replaying the "
                    "archive could make results drift from the thesis snapshot "
                    "if any FPDS records have been revised since.)")),
...
def run_stage(st: Stage, no_backfill: bool) -> None:
    ...
    if st.key == "backfill_replay":
        ...
        if no_backfill:
            ...
            print("  !! the parquet will be missing archived DLA rows.")
            return
        if len(zips) != 1:
