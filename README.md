...
| `data_defs/` | reference codebook for FPDS place-of-manufacture codes |
...
| Data | Size | Where |
|---|---|---|
| `raw_data/procurement/FY*_All_Contracts_Full_*.zip` (10 zips) + `api_backfill/*.zip` (1 zip) | ~17 GB | **Archived data deposit** (DOI: to be added). A fresh USAspending bulk download may not match exactly — if any FPDS records have been revised since, the archived snapshot is the reference for an exact reproduction. |
...
| `donor_universe` | candidate control (donor) NSNs, used by matched + synth |
