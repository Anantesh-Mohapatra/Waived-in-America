...
# JSON -> generator-shape transforms live in es_stats (single owner, shared
...
def build_per_item_domestic_table(paths: dict, out_dir: Path) -> Path | None:
    """Per-item domestic-sourcing effect for the (tiny) non-container sample:
    the synth demeaned effect at each post-waiver event year and the matched
    per-item τ in both post windows. Built only for the non-container check."""
...
def _sanitize_undefined_se(path: Path) -> None:
    ...
    instead of the literal 'nan' that float formatting produces. The N column
    still shows the single observation."""
    if not path.exists():
        return
...
def build(variant: str) -> None:
    """Build whatever designs have inputs present in this variant's results
    tree; re-running after more inputs land fills in the rest."""
    ...
    # Non-container check: per-item domestic-sourcing table (per-item breakdown
    # of the long-term aggregate).
