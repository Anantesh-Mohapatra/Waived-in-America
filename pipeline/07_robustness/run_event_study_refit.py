...
def fit_outcome(panel_sub: pl.DataFrame, outcome: str, cache_dir: Path,
    ...
    es_coefs = {
        ...
        # (the shape the thesis ES_COEFS structure expects).
...
def run_dla_only() -> None:
    ...
    Treated units carry DLA + overlay transaction histories, while the controls
    are almost entirely DLA-only. This re-fits the event study on the
    DLA-only panel, where the overlay is removed from every unit. Dropping the
    overlay also drops the overlay-dependent treated units (some treated NSNs
    fall out under the standard pre-window filter)."""
    dla_panel = "dla_only"
...
def run_main() -> None:
    ...
    figure/ladder/summary generators (build_thesis_{figures,tables})."""
