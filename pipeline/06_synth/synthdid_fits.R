...
# aggregated by build_donor_pools.py). Each pool has rows = balanced units
...
# NA for single-treated panels. Only `placebo_se` is defined for N1=1. We use
...
#
# CLI:
#   Rscript synthdid_fits.R              # full run
#   Rscript synthdid_fits.R --trial      # trial run, 2 NSNs only
#   Rscript synthdid_fits.R --workers=N  # override worker count
...
#   synth[t] = omega' Y_donors[, t]   (the DID intercept is absorbed via the
#   demeaning below)
...
main <- function() {
  ...
  if (trial_mode) {
    ...
    # Stage 2 (fast trial). Non-container = 1680016229189 (chosen to exercise
