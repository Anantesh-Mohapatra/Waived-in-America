...
Some treated NSNs drop here: a treated NSN with no pre-period observations
under the day-precise event-year clock is excluded, and a treated NSN with no
pre-period observations on a specific outcome is excluded for that outcome only.
...
def main() -> None:
    ...
    # ----- load -----
    # Lazy scan: keeps peak RAM to ~one anchor at a time.
    needed_cols = [
    ...
    # filters would otherwise leave ghost pools that downstream stages silently
    # pick up.
