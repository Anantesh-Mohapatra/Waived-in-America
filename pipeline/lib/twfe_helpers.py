...
# pyfixest auto-drops singletons anyway; pre-filtering reduces the demean work.
MIN_OBS_PER_NSN = 2
...
# does not match are treated as stale and refit.
...
def apply_pre_window_filter(
    ...
    pre-window), so the resulting samples are not identical.
