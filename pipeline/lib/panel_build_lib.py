...
def load_dla(
    ...
    del dla_arrow
    if log_filter:
        log_filter("dla_action_date_cutoff", "DLA full", len(dla),
    ...
    if len(anomalies) > 0:
        if anomalies_out is not None:
            # Sort for deterministic CSV row order (group_by order is arbitrary).
...
def proxy_unit_price_recovery(
    ...
    """For BidLink rows where bl_price = 0, recover
...
def aggregate_to_panel(
    ...
    # FPDS-NG v1.4 element 10A (extent_competed_code); FAR Part 6.
    #   competed     = {A, D, F, CDO}
