...
def collect_candidate_piids(df: pl.DataFrame) -> list[str]:
    ...
    3. bl_order as-is when it's a short suffix (parent-constrained join)
    4. bl_contract + bl_order when Order is a short suffix (concat form)
    """
    contracts = (
    ...
    # Short Order codes as-is (for the parent-constrained join)
    short_orders = (
        df.filter(_order_is_short_suffix_expr())
        ["bl_order"].unique().to_list()
    )
    # Concat form: bl_contract + bl_order for short Order codes
    short_concats = (
        df.filter(_order_is_short_suffix_expr())
...
def load_fpds_for_piids(
    ...
    so modification rows don't multiply the join with BidLink line items.

    Also fetches parent_award_id_piid (needed for the parent-constrained join).
    """
...
def enrich_with_fpds(
    ...
    the FPDS side (used only by the parent-constrained join, not needed in output).
    """
...
def enrich_unmatched_via_parent(
    ...
    """Second-pass enrichment for short order codes recorded under a parent award.

    For unmatched rows where bl_order is a short suffix, join on
    award_id_piid = bl_order AND parent_award_id_piid = bl_contract — orders
    that appear in FPDS keyed by the short order code (e.g., 'X38V') with the
    parent award as parent_award_id_piid.
    """
    ...
    # Update join metadata for rows that matched via the parent-award join
    rejoined = rejoined.with_columns(
