...
panel_enriched.parquet anchors event_year to each NSN's own first_waiver_date,
so donor rows carry a constant sentinel and lose the day-precision the
matched-controls DiD needs. This re-anchors every NSN to each treated's date,
giving a day-precise event_year for the donor side of every (treated, donor) pair.
...
[-5, +10] per anchor. The overlay conditional is the only intended behavioral
