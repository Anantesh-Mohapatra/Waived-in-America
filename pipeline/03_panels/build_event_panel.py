...
as the base and a transaction-record enrichment overlay for the NSNs with
BidLink line-item coverage (treated + controls).
...
# Proxy unit-price recovery: when BidLink reports bl_price = 0, we can
# recover a unit price as
...
# Otherwise 0 (instant-only individual waivers, or no matched waiver).
...
universe = (
    ...
    # Deterministic row order for the committed CSV (.unique() order is unstable).
