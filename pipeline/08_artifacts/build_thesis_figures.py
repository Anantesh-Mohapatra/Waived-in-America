...
def plot_nn_did_dotplot(outcome: str, post_label: str = "headline", *,
    ...
    # at a glance. Per-NSN points have no individual CIs.
...
# domestic) by fiscal year. Computed from the same _compute_foreign_share()
# that builds Table A0, so the figure and table can
...
# Stacked bottom -> top: U.S. on the floor, China on the ceiling — the two
# series of interest placed at the chart edges.
...
ORIGIN_COLORS = {
    ...
    "China":         _deep[3],        # red -- the band of interest
}
...
def plot_treatment_origin_by_fy() -> Path:
    ...
    tab = (agg.join(tot, on="fy")
              ...
              .sort(["fy", "origin"]))
    # Round for a byte-stable committed CSV (parallel summation wobbles the last
    # ulp; cents for dollars and 12 dp for shares are below meaningful precision).
    tab.with_columns(
    ...
    for ax, mat, title, toplab in panels:
        ...
        for o in present:
            ...
            ax.bar(x, vals, 0.82, bottom=bottom, label=o, color=ORIGIN_COLORS[o],
                   edgecolor="white", linewidth=0.5)
            bottom = bottom + vals
        # Per-year magnitude above each bar (transactions in A, amount paid in B).
