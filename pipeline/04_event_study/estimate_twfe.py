...
# - `domestic_share` — share of the cell's manufacture-code-reporting transactions with `place_of_manufacture_code = 'D'`
#
...
# identify post-treatment FY FE). Also applies `MIN_OBS_PER_NSN ≥ 2`.
#
# **Plot truncation**: x-axis shown for `ey ∈ [-4, +3]` (full range estimated;
# the far pre-period leads are identified off very few NSNs, so they're shown in
# the table but truncated from the plot). N treated NSNs per event year shown as
# faint bars.
#
# **Architecture**: regressions run in parallel via `joblib.Parallel(n_jobs=4)`
# with disk-cached fitted models. Set `FORCE_REFRESH=True` to refit everything.
