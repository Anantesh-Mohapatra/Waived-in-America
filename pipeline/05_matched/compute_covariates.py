...
donor_universe.csv), apply the strict-coverage rule:
  ...
  - If treated has no active EY in [-5, -1] the treated is dropped (no
    pre-period observations to anchor matching).
...
# 1 diagnostic-only covariate (mean_competitive_share). Stage 3 matches on
# the first four only — its own COVS list selects from this set.
COVARIATES = (
    "log_n_transactions_per_fy",
...
def main() -> None:
    ...
    # Order columns deterministically: identifiers, then the covariate columns,
    ...
    print(f"  pairs with all 4 matching covariates defined "
