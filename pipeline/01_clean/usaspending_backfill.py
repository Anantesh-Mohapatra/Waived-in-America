...
----------------------
The USAspending bulk-archive zips under-cover the most recent months relative
to the live API (they appear to lag DoD reporting by a few months). The backfill
merges the archived API rows in to close that gap; without it,
dla_contract_enriched_*.parquet is nearly empty for those months.
...
Scope notes
-----------
- IDV records are not backfilled: the bulk archive is contracts-only, and
  adding IDVs would break the "every parquet row is a money transaction"
  invariant downstream code relies on.
- Only DLA rows are backfilled: the DLA contract-history join is the only
  downstream consumer affected by the gap.
- Feb-Mar 2026 DLA rows were not yet in FPDS at the archived snapshot.

Known data limit
----------------
Some DLA contract-history PIIDs never match FPDS. Even with this backfill,
