...
Procurement history covers: 2018 and 2026 only (the only years in the FOIA release).

Quirks handled:
  - Three different zip naming conventions across years
...
def cast_contract_types(df: pl.DataFrame) -> pl.DataFrame:
    ...
    return df.with_columns(
        ...
        # Rebuild NSN = FSC || NIIN. Source FSC and NSN disagree for
        # FSC-reclassified items; cross-referencing BidLink and nsnlookup shows
        # FSC is the correct one (source NSN keeps a stale prefix), and without
        # the rebuild NSN-keyed joins silently drop them. Matches cast_proc_types
        # (procurement branch).
...
def _usa_scan() -> pl.LazyFrame:
    ...
    DLA CONTRACT_NUMBERs map to USAspending in three ways:
      ...
      - 13-char delivery order -> award_id_piid directly
        (the CONTRACT_NUMBER is the bare 13-char order PIID, without the parent prefix)
...
def _build_usa_mod_lookup(dla_piids: pl.Series) -> pl.DataFrame:
    ...
    Without filtering, USAspending has millions of (piid, mod) pairs across
    all agencies. Pre-filtering to DLA-matchable PIIDs keeps memory manageable.
