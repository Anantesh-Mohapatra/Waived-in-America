...
for zp in zip_paths:
    ...
    with zipfile.ZipFile(zp) as zf:
        ...
        for csv_idx, name in enumerate(csv_files, start=1):
            ...
            df = df.filter(pl.col("place_of_manufacture_code") != "C") # drop rows with "C" in place_of_manufacture_code, which is "Not a manufactured end product"
            if df.is_empty():
                continue
