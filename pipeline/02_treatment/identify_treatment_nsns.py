...
#
# **Method**: BidLink procurement history only.
#
...
#
# Starting point: the unique NSNs extracted from madeinamerica.gov waivers.
...
# (NSN or part number) to its waiver record and `created` timestamp, which we
# use as the waiver date. We filter to NSN-type identifiers and take the
# earliest `created` date per NSN.
...
# ### Drop Summary
#
# A complete accounting of all waiver NSNs and their disposition.
...
print(f"Disposition of all {len(nsn_ref)} waiver NSNs:")
...
# Add treatment-timing context so downstream consumers can use the panel
...
# %%
# Unmatched contracts: BidLink rows that did not join to the procurement parquet.
unmatched = (
    treatment_panel
    .filter(~pl.col("matched_to_parquet"))
