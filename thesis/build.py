...
# the analysis. The manuscript caption is kept (correct table
...
# the manuscript is re-exported, so a stale anchor emits a warning.
WIDOW_ANCHORS = [
...
# Unicode characters Latin Modern Roman lacks a glyph for. newunicodechar maps each
...
def check_glyph_coverage(clean: str) -> None:
    """Warn if clean.md contains a non-ASCII char that Latin Modern Roman lacks and
...
def tex_escape_plain(s: str) -> str:
    ...
    through to xelatex (Latin Modern covers it); check mark -> math \\checkmark."""
...
def split_paragraphs(text: str) -> str:
    ...
    one block joined by \\\\ -- and \\parskip does not apply. Insert a blank line
